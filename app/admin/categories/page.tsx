...
const COLOR_LIST = [
  { name: 'Indigo', bg: 'bg-indigo-50 dark:bg-indigo-950/60 border-indigo-100 dark:border-indigo-900 text-indigo-600 dark:text-indigo-400', hex: '#6366f1' },
  { name: 'Rose', bg: 'bg-rose-50 dark:bg-rose-950/60 border-rose-100 dark:border-rose-900 text-rose-600 dark:text-rose-400', hex: '#f43f5e' },
  { name: 'Emerald', bg: 'bg-emerald-50 dark:bg-emerald-950/60 border-emerald-100 dark:border-emerald-900 text-emerald-600 dark:text-emerald-400', hex: '#10b981' },
  { name: 'Amber', bg: 'bg-amber-50 dark:bg-amber-950/60 border-amber-100 dark:border-amber-900 text-amber-600 dark:text-amber-400', hex: '#f59e0b' },
  { name: 'Blue', bg: 'bg-blue-50 dark:bg-blue-950/60 border-blue-100 dark:border-blue-900 text-blue-600 dark:text-blue-400', hex: '#3b82f6' },
  { name: 'Violet', bg: 'bg-violet-50 dark:bg-violet-950/60 border-violet-100 dark:border-violet-900 text-violet-600 dark:text-violet-400', hex: '#8b5cf6' },
  { name: 'Pink', bg: 'bg-pink-50 dark:bg-pink-950/60 border-pink-100 dark:border-pink-900 text-pink-600 dark:text-pink-400', hex: '#ec4899' },
  { name: 'Cyan', bg: 'bg-cyan-50 dark:bg-cyan-950/60 border-cyan-100 dark:border-cyan-900 text-cyan-600 dark:text-cyan-400', hex: '#06b6d4' },
];
...
export default function AdminCategoriesPage() {
  ...
  const handleStartEdit = (cat: Category) => {
    ...
    setSelectedColor(cat.color || 'Indigo');
  };
  ...
  const handleSubmitCategory = async (e: React.FormEvent) => {
    ...
    try {
      const supabase = createClient();
      const slug = catSlug.trim() || catName.toLowerCase().replace(/\s+/g, '-').replace(/[^a-z0-9-]/g, '');
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between bg-white dark:bg-slate-900 p-5 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400">Atur nama, slug, ikon, dan tema warna kategori platform.</p>
          </div>
        </div>
      ...
      <form onSubmit={handleSubmitCategory} className="p-6 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-sm space-y-5">
        <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-800 pb-3">
          <h4 className="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
            {editingCatId ? <Edit3 className="w-4 h-4 text-indigo-600 dark:text-indigo-400" /> : <Plus className="w-4 h-4 text-indigo-600 dark:text-indigo-400" />}
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Nama Kategori <span className="text-rose-500">*</span></label>
            <input
              type="text"
          ...
          <div className="space-y-1">
            <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Slug (URL Keyword)</label>
            <input
              type="text"
        ...
        <div className="space-y-2">
          <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Pilih Ikon Kategori</label>
        ...
        {/* Color Selection */}
        <div className="space-y-2">
          <label className="text-xs font-semibold text-slate-700 dark:text-slate-300">Pilih Warna Tema Card</label>
        ...
        <div className="flex justify-end gap-2 pt-2 border-t border-slate-100 dark:border-slate-800">
          ...
          >
            ...
            <span>{submitting ? 'Menyimpan...' : editingCatId ? 'Perbarui Kategori' : 'Simpan Kategori Baru'}</span>
          </button>
        </div>
      </form>
          ...
          return (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-center gap-3">
                  <div className={`w-11 h-11 rounded-xl flex items-center justify-center border shrink-0 ${colorClass}`}>
                  ...
                  <div>
                    ...
                    <p className="text-xs text-indigo-600 dark:text-indigo-400 font-mono font-medium">#{cat.slug}</p>
                  </div>
                </div>
