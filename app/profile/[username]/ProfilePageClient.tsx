...
export default function ProfilePageClient({params}: ProfilePageClientProps) {
  ...
  const handleFollowToggle = async () => {
    ...
    try {
      ...
      const message = err instanceof Error ? err.message : "Gagal mengubah status mengikuti";
      toast.error(message);
    }
  };
    ...
    return (
      <div className="text-center py-16 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl p-8 shadow-sm">
        ...
        <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
          Pengguna dengan nama @{username} tidak terdaftar.
        </p>
      </div>
    );
  }
  ...
  const userCategories = Array.from(
    new Map(
      quotes.filter((q) => q.category?.id && q.category?.name).map((q) => [q.category!.id, q.category!]),
    ).values(),
  );
  ...
  return (
    <div className="space-y-6 pb-12">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-3">
              <div>
                <div className="flex items-center justify-center sm:justify-start gap-2">
                  ...
                  {profile.role === "admin" && (
                    <span className="px-2.5 py-0.5 text-xs font-semibold text-indigo-700 dark:text-indigo-300 bg-indigo-50 dark:bg-indigo-950/80 rounded-full border border-indigo-200 dark:border-indigo-800">
                      Admin
                    </span>
                  )}
                </div>
                <p className="text-sm text-indigo-600 dark:text-indigo-400 font-mono font-medium">
                  @{profile.username}
                </p>
              </div>
          ...
          {filteredQuotes.length === 0 ? (
            <div className="text-center py-12 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-6 shadow-sm">
              <p className="text-xs text-slate-500 dark:text-slate-400">
                Tidak ada kutipan yang sesuai dengan filter ini.
              </p>
            </div>
          ) : (
      ...
      {activeTab === "badges" && <AchievementBadges profile={profile} />}

      {activeTab === "analytics" && <ProfileAnalytics profile={profile} quotes={quotes} />}
