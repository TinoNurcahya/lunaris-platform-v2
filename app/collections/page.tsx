    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="relative overflow-hidden rounded-3xl bg-indigo-600 p-8 sm:p-10 text-white shadow-lg">
        <div className="relative z-10 space-y-3">
          ...
          <h1 className="text-2xl sm:text-4xl font-bold tracking-tight">Jelajah Koleksi</h1>

          <p className="text-xs sm:text-sm text-indigo-100 max-w-xl leading-relaxed">
            Temukan koleksi kutipan yang dirangkai khusus berdasarkan tema, suasana hati, dan musik favorit
            oleh komunitas Lunarys.
          </p>
        </div>
      </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="h-44 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 animate-pulse"
            />
          ))}
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
          {activeCollections.map((col) => (
            <Link
              ...
              <div
                className={`h-24 ${normalizeCoverColor(col.cover_gradient)} p-4 flex items-start justify-between text-white relative`}>
                ...
                {col.user && (
                  <div className="flex items-center gap-2 pt-2 border-t border-slate-100 dark:border-slate-800/80">
                    <img
                      src={
                        col.user.avatar_url ||
                        `https://api.dicebear.com/7.x/bottts/svg?seed=${col.user.username}`
                      }
                      alt={col.user.name}
