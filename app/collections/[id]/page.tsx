...
export default function CollectionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="space-y-8 pb-12">
      ...
      <div className={`relative overflow-hidden rounded-3xl bg-gradient-to-r ${collection.cover_gradient} p-8 sm:p-10 text-white shadow-lg space-y-4`}>
        ...
        {collection.user && (
          <div className="flex items-center gap-3 pt-2 border-t border-white/20">
            <img
              src={collection.user.avatar_url || `https://api.dicebear.com/7.x/bottts/svg?seed=${collection.user.username}`}
        ...
        {!collection.quotes || collection.quotes.length === 0 ? (
          <div className="text-center py-12 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm">
            <p className="text-xs text-slate-500 dark:text-slate-400">
              Belum ada kutipan dalam koleksi ini.
            </p>
          </div>
        ) : (
