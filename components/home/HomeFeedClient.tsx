...
export default function HomeFeedClient({initialQuotes, initialCategories}: HomeFeedClientProps) {
  ...
  const loadMore = async () => {
    ...
    const moreQuotes = await fetchQuotes({
      categoryId: selectedCategory,
      sortBy,
      mood: selectedMood,
      page: nextPage,
  ...
  return (
    <div className="space-y-8 pb-12 min-w-0 w-full max-w-full">
      ...
      {quoteOfTheDay && !selectedCategory && (
        <div className="relative overflow-hidden rounded-2xl bg-indigo-600 p-6 sm:p-8 shadow-lg text-white">
      ...
      <div className="space-y-3 bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-4 sm:p-5 shadow-sm transition-colors duration-200">
        <div className="flex items-center justify-between">
          ...
          <Link
            href="/categories"
            className="text-xs sm:text-sm text-indigo-600 dark:text-indigo-400 hover:underline font-semibold shrink-0 transition-colors duration-200">
            Lihat Semua →
          </Link>
        </div>
      ...
      {loading ? (
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="h-44 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 animate-pulse transition-colors duration-200"
            />
          ))}
        </div>
      ) : quotes.length === 0 ? (
        <div className="text-center py-16 bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-8 shadow-sm transition-colors duration-200">
          ...
          <h3 className="text-base font-bold text-slate-800 dark:text-slate-200 transition-colors duration-200">
            Belum Ada Kutipan
          </h3>
          <p className="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-1 mb-4 transition-colors duration-200">
            Belum ada kutipan pada kategori ini.
          </p>
          <Link
            href="/quotes/create"
