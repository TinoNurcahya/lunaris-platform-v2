...
import {Search, Folder, X, ExternalLink} from "lucide-react";
    ...
    <>
      {parts.map((part, index) =>
        part.toLowerCase() === query.trim().toLowerCase() ? (
          <mark
            key={index}
            className="bg-indigo-200 dark:bg-indigo-900/90 text-indigo-950 dark:text-indigo-100 font-bold px-0.5 rounded">
            {part}
          </mark>
        ) : (
          part
...
export default function CommandPalette({isOpen, onClose}: CommandPaletteProps) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      const [{data: quotesData}, {data: usersData}, {data: categoriesData}] =
        await Promise.all([
          supabase
            .from("quotes")
  ...
  return (
    <div
      onClick={onClose}
      className="fixed inset-0 z-50 flex items-start justify-center pt-8 sm:pt-24 px-3 sm:px-4 bg-slate-900/60 backdrop-blur-xs animate-in fade-in duration-150">
      <div
        onClick={(e) => e.stopPropagation()}
        className="w-full max-w-xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl sm:rounded-3xl shadow-2xl overflow-hidden flex flex-col max-h-[85vh] my-auto">
        {/* Input Bar */}
        <form
          onSubmit={handleFullSearch}
          className="flex items-center px-4 border-b border-slate-200 dark:border-slate-800 shrink-0">
                  ...
                  <div className="space-y-1">
                    {quotes.map((q) => (
                      <button
                        ...
                        <p className="text-[10px] text-slate-400">oleh {q.user?.name || "Anonim"}</p>
                      </button>
                    ))}
                  </div>
                </div>
