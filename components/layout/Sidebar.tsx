...
export default function Sidebar({profile: propProfile, isAdmin: propIsAdmin}: SidebarProps) {
  ...
  const mainNavItems = [
    ...
    },
    {
      ...
      badgeColor:
        "bg-indigo-100 dark:bg-indigo-900/60 text-indigo-800 dark:text-indigo-200 border border-indigo-200 dark:border-indigo-700",
      emptyText: "Disimpan",
    },
    {
      label: "Notifikasi",
  ...
  const adminNavItems = [
    {
      ...
      badgeText: "Insight",
      badgeColor:
        "bg-indigo-200/80 dark:bg-indigo-900/80 text-indigo-900 dark:text-indigo-200 border border-indigo-300 dark:border-indigo-700",
    },
    {
    ...
    },
    {
      ...
      badgeColor:
        "bg-indigo-100 dark:bg-indigo-950 text-indigo-700 dark:text-indigo-300 font-mono border border-indigo-200 dark:border-indigo-800",
    },
    {
      ...
      badgeColor:
        "bg-sky-100 dark:bg-sky-950 text-sky-700 dark:text-sky-300 font-mono border border-sky-200 dark:border-sky-800",
    },
    {
    ...
    },
    {
      ...
      icon: ShieldCheck,
      badgeText: "Security",
      badgeColor: "bg-emerald-600 text-white font-mono text-[10px]",
    },
  ];
  ...
  return (
    <aside className="w-64 hidden md:block shrink-0">
      <div className="sticky top-20 space-y-4 max-h-[calc(100vh-6rem)] overflow-y-auto custom-scrollbar pr-1">
          ...
          <div className="bg-white dark:bg-slate-900 border border-slate-200/90 dark:border-slate-800 rounded-2xl p-4 shadow-sm space-y-3 transition-colors duration-200">
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0 flex-1">
                <h4 className="text-base font-bold text-slate-900 dark:text-white truncate transition-colors duration-200">
                  {profile.name}
                </h4>
                <p className="text-xs text-indigo-600 dark:text-indigo-400 font-mono font-medium transition-colors duration-200">
                  @{profile.username}
                </p>
              ...
              return (
                <Link
                  ...
                  <div className="flex items-center gap-3 min-w-0">
                    <Icon
                      className={`w-4 h-4 shrink-0 ${isActive ? "text-indigo-600 dark:text-indigo-400" : "text-slate-400 dark:text-slate-500"}`}
                    />
                  ...
                  {badge ? (
                    <span
                      className={`px-2 py-0.5 text-[10px] font-bold font-mono rounded-full shrink-0 ${item.badgeColor || "bg-indigo-600 text-white"}`}>
                      {badge}
                    </span>
                  ) : item.emptyText ? (
                    <span
                      className={`px-2 py-0.5 text-[10px] font-medium rounded-full shrink-0 ${item.badgeColor}`}>
                      {item.emptyText}
                    </span>
                  ) : null}
                </Link>
        ...
        {isAdmin && (
          <div className="bg-indigo-50 dark:bg-slate-900 border border-indigo-300/70 dark:border-indigo-700/60 rounded-2xl p-3.5 shadow-sm space-y-3">
            ...
            <div className="flex items-center justify-between px-1">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <h4 className="text-xs font-bold text-indigo-950 dark:text-indigo-200 tracking-tight">
                    Admin Control Center
                  </h4>
                  <div className="flex items-center gap-1 text-[10px] font-semibold text-sky-700 dark:text-sky-400">
              ...
              {totalAdminPending > 0 ? (
                <span className="px-2 py-0.5 text-[10px] font-bold text-white bg-sky-500 rounded-full animate-pulse shadow-xs">
                  {formatBadgeCount(totalAdminPending)} Aksi
                </span>
              ) : (
                <span className="px-2 py-0.5 text-[10px] font-bold text-indigo-800 dark:text-indigo-300 bg-indigo-200/80 dark:bg-indigo-900/60 rounded-full border border-indigo-300 dark:border-indigo-700">
                  Admin
                </span>
              )}
            </div>
            ...
            <nav className="space-y-1 pt-1">
              {adminNavItems.map((item) => {
                ...
                const isActive = pathname === item.href;
                const badge =
                  item.badgeCount !== undefined ? formatBadgeCount(item.badgeCount) : item.badgeText;
                  ...
                  <Link
                    ...
                    className={`flex items-center justify-between px-3 py-2 text-xs font-semibold rounded-xl transition-all ${
                      isActive
                        ...
                        : "text-indigo-950 dark:text-indigo-200 hover:bg-indigo-200/60 dark:hover:bg-indigo-900/40"
                    }`}>
                    <div className="flex items-center gap-2.5 min-w-0">
                      <Icon
                        className={`w-4 h-4 shrink-0 ${isActive ? "text-white" : "text-indigo-700 dark:text-indigo-400"}`}
                      />
                      <span className="truncate">{item.label}</span>
                    </div>
                    ...
                    {badge ? (
                      ...
                      <span
                        className={`px-2 py-0.5 text-[10px] font-semibold rounded-full shrink-0 ${item.badgeColor}`}>
                        {item.emptyText}
                      </span>
                    ) : null}
                  </Link>
        ...
        {/* Call to Action Box */}
        <div className="bg-indigo-50 dark:bg-slate-900 border border-indigo-100 dark:border-indigo-900/50 rounded-2xl p-5 shadow-sm">
